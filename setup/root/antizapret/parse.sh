...
ANTIZAPRET_WG_IP6="${ANTIZAPRET_WG_IP6:-fd29:0:0:8::1}"

# Оставляем только корректные IPv6-подсети
filterIPv6() {
	grep -E '^[0-9A-Fa-f:]+/[0-9]{1,3}$' | awk -F '/' '$2 >= 1 && $2 <= 128 {print}'
}
...
if [[ -z "$1" || "$1" == 'ip' || "$1" == 'ips' || "$1" == 'noclear' || "$1" == 'noclean' ]]; then
	...
	filterIPv6 < temp/route-ips.txt > result/route-ips6.txt

	# Выводим результат
	echo "$(wc -l < result/route-ips6.txt) - route-ips6.txt"

	# Обрабатываем список запрещенных сетей для форвардинга
	sed -E 's/[\r[:space:]]+//g; /^[[:punct:]]/d; /^$/d' config/*drop-ips.txt | sort -u > temp/drop-ips.txt
	awk -F'[/.]' 'NF==5 && $1>=0 && $1<=255 && $2>=0 && $2<=255 && $3>=0 && $3<=255 && $4>=0 && $4<=255 && $5>=1 && $5<=32 {print}' temp/drop-ips.txt > result/drop-ips.txt
	filterIPv6 < temp/drop-ips.txt > result/drop-ips6.txt

	# Выводим результат
	echo "$(wc -l < result/drop-ips.txt) - drop-ips.txt"
	echo "$(wc -l < result/drop-ips6.txt) - drop-ips6.txt"

	# Обновляем ipset antizapret-drop и antizapret-drop6
	{
		...
		while read -r cidr; do
			echo "add antizapret-drop6 $cidr -exist"
		done < result/drop-ips6.txt
	} | ipset restore

	# Обрабатываем список IP-источников, которым запрещены входящие подключения к серверу (INPUT)
	sed -E 's/[\r[:space:]]+//g; /^[[:punct:]]/d; /^$/d' config/*deny-ips.txt | sort -u > temp/deny-ips.txt
	awk -F'[/.]' 'NF==5 && $1>=0 && $1<=255 && $2>=0 && $2<=255 && $3>=0 && $3<=255 && $4>=0 && $4<=255 && $5>=1 && $5<=32 {print}' temp/deny-ips.txt > result/deny-ips.txt
	filterIPv6 < temp/deny-ips.txt > result/deny-ips6.txt

	# Выводим результат
	echo "$(wc -l < result/deny-ips.txt) - deny-ips.txt"
	echo "$(wc -l < result/deny-ips6.txt) - deny-ips6.txt"

	# Обновляем ipset antizapret-deny и antizapret-deny6
	{
		...
		while read -r cidr; do
			echo "add antizapret-deny6 $cidr -exist"
	...
	# Добавляем IPv6-маршруты
	if [[ "$IPV6" == 'y' ]]; then
		echo "push \"route-ipv6 $FAKE_NET6\"" >> result/DEFAULT
		echo "route-ipv6 $FAKE_NET6" >> result/tp-link-openvpn-routes.txt
		echo "/ipv6 route add dst-address=$FAKE_NET6 gateway=$ANTIZAPRET_WG_IP6 distance=1 comment=\"antizapret-wireguard\"" >> result/mikrotik-wireguard-routes.txt
		while read -r cidr; do
			echo "push \"route-ipv6 $cidr\"" >> result/DEFAULT
			echo "route-ipv6 $cidr" >> result/tp-link-openvpn-routes.txt
			echo "/ipv6 route add dst-address=$cidr gateway=$ANTIZAPRET_WG_IP6 distance=1 comment=\"antizapret-wireguard\"" >> result/mikrotik-wireguard-routes.txt
		done < result/route-ips6.txt
	fi
	...
	# Создаем файл ips6 для WireGuard/AmneziaWG
	if [[ "$IPV6" == 'y' ]]; then
		echo -n ", $FAKE_NET6" > result/ips6
		awk '{printf ", %s", $0}' result/route-ips6.txt >> result/ips6
	else
		> result/ips6
	fi
	...
	# Обновляем файл ips6 в WireGuard/AmneziaWG только если файл изменился
	if [[ -f result/ips6 ]] && ! diff -q result/ips6 /etc/wireguard/ips6; then
		cp -f result/ips6 /etc/wireguard/ips6
	fi
	...
	if [[ "$RESTRICT_FORWARD" == 'y' ]]; then
		...
		filterIPv6 < temp/forward-ips.txt > result/forward-ips6.txt

		# Выводим результат
		echo "$(wc -l < result/forward-ips.txt) - forward-ips.txt"
		echo "$(wc -l < result/forward-ips6.txt) - forward-ips6.txt"

		# Обновляем ipset antizapret-forward и antizapret-forward6
		{
			...
			while read -r cidr; do
				echo "add antizapret-forward6 $cidr -exist"
	...
	if [[ "$ATTACK_PROTECTION" == 'y' ]]; then
		# Обрабатываем конфигурационные файлы
		sed -E 's/[\r[:space:]]+//g; /^[[:punct:]]/d; /^$/d' config/*allow-ips.txt | sort -u > temp/allow-ips.txt
		awk -F'[/.]' 'NF==5 && $1>=0 && $1<=255 && $2>=0 && $2<=255 && $3>=0 && $3<=255 && $4>=0 && $4<=255 && $5>=1 && $5<=32 {print}' temp/allow-ips.txt > result/allow-ips.txt
		filterIPv6 < temp/allow-ips.txt > result/allow-ips6.txt

		# Выводим результат
		echo "$(wc -l < result/allow-ips.txt) - allow-ips.txt"
		echo "$(wc -l < result/allow-ips6.txt) - allow-ips6.txt"

		# Обновляем ipset antizapret-allow и antizapret-allow6
		{
			...
			while read -r cidr; do
				echo "add antizapret-allow6 $cidr -exist"
