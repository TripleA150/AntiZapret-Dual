...
if [[ "$IPV6" == 'y' ]]; then
	sysctl -qw net.ipv6.conf.all.forwarding=1 || true
	sysctl -qw net.ipv6.conf.$DEFAULT_INTERFACE.accept_ra=2 || true
	if [[ -z "$DEFAULT_IP6" ]]; then
		DEFAULT_IP6="$(ip -6 route get 2606:4700:4700::1111 2>/dev/null | grep -oP 'src \K\S+')"
		if [[ -z "$DEFAULT_IP6" ]]; then
			echo 'Default IPv6 address not found! IPv6 is available only inside VPN'
		fi
	fi
fi
...
if [[ "$ANTIZAPRET_WARP" == 'y' ]]; then
	...
	if [[ -z "$ANTIZAPRET_WARP_PRIVATE_KEY" || -z "$ANTIZAPRET_WARP_PUBLIC_KEY" || -z "$ANTIZAPRET_WARP_ENDPOINT" || -z "$ANTIZAPRET_WARP_ADDRESS" || ( "$IPV6" == 'y' && -z "$ANTIZAPRET_WARP_ADDRESS6" ) ]]; then
		...
		ANTIZAPRET_WARP_ADDRESS6="$(echo "$REG" | jq -r '.config.interface.addresses.v6')/128"
	fi

	# Выпускаем IPv6 через WARP
	ANTIZAPRET_WARP_ADDRESSES="$ANTIZAPRET_WARP_ADDRESS"
	ANTIZAPRET_WARP_ALLOWED_IPS='0.0.0.0/0'
	ANTIZAPRET_WARP_RULES6=
	if [[ "$IPV6" == 'y' && -n "$ANTIZAPRET_WARP_ADDRESS6" ]]; then
		ANTIZAPRET_WARP_ADDRESSES="$ANTIZAPRET_WARP_ADDRESS, $ANTIZAPRET_WARP_ADDRESS6"
		ANTIZAPRET_WARP_ALLOWED_IPS='0.0.0.0/0, ::/0'
		ANTIZAPRET_WARP_RULES6="PostUp = ip -6 rule add from $ANTIZAPRET_NET6 to $ANTIZAPRET_NET6 lookup main priority 5000 || true
PostUp = ip -6 rule add from $ANTIZAPRET_NET6 to $FAKE_NET6 lookup main priority 5000 || true
PostUp = ip -6 rule add from $ANTIZAPRET_NET6 lookup 13335 priority 10000 || true
PostDown = ip -6 rule del from $ANTIZAPRET_NET6 to $ANTIZAPRET_NET6 priority 5000
PostDown = ip -6 rule del from $ANTIZAPRET_NET6 to $FAKE_NET6 priority 5000
PostDown = ip -6 rule del from $ANTIZAPRET_NET6 lookup 13335 priority 10000
"
	fi

	echo "[Interface]
PrivateKey = $ANTIZAPRET_WARP_PRIVATE_KEY
Address = $ANTIZAPRET_WARP_ADDRESSES
MTU = 1420
Table = 13335
...
AllowedIPs = $ANTIZAPRET_WARP_ALLOWED_IPS
...
Endpoint = $ANTIZAPRET_WARP_ENDPOINT" > $ANTIZAPRET_WARP_PATH
	...
	if [[ $? -eq 0 ]]; then
		...
		if [[ "$IPV6" == 'y' && -n "$ANTIZAPRET_WARP_ADDRESS6" ]]; then
			ANTIZAPRET_OUT_IP6="${ANTIZAPRET_WARP_ADDRESS6%%/*}"
		fi
	else
...
if [[ "$VPN_WARP" == 'y' ]]; then
	...
	if [[ -z "$VPN_WARP_PRIVATE_KEY" || -z "$VPN_WARP_PUBLIC_KEY" || -z "$VPN_WARP_ENDPOINT" || -z "$VPN_WARP_ADDRESS" || ( "$IPV6" == 'y' && -z "$VPN_WARP_ADDRESS6" ) ]]; then
		...
		VPN_WARP_ADDRESS6="$(echo "$REG" | jq -r '.config.interface.addresses.v6')/128"
	fi

	# Выпускаем IPv6 через WARP
	VPN_WARP_ADDRESSES="$VPN_WARP_ADDRESS"
	VPN_WARP_ALLOWED_IPS='0.0.0.0/0'
	VPN_WARP_RULES6=
	if [[ "$IPV6" == 'y' && -n "$VPN_WARP_ADDRESS6" ]]; then
		VPN_WARP_ADDRESSES="$VPN_WARP_ADDRESS, $VPN_WARP_ADDRESS6"
		VPN_WARP_ALLOWED_IPS='0.0.0.0/0, ::/0'
		VPN_WARP_RULES6="PostUp = ip -6 rule add from $VPN_NET6 to $VPN_NET6 lookup main priority 5000 || true
PostUp = ip -6 rule add from $VPN_NET6 lookup 13336 priority 10000 || true
PostDown = ip -6 rule del from $VPN_NET6 to $VPN_NET6 priority 5000
PostDown = ip -6 rule del from $VPN_NET6 lookup 13336 priority 10000
"
	fi

	echo "[Interface]
PrivateKey = $VPN_WARP_PRIVATE_KEY
Address = $VPN_WARP_ADDRESSES
MTU = 1420
Table = 13336
...
AllowedIPs = $VPN_WARP_ALLOWED_IPS
PersistentKeepalive = 15
...
if [[ "$TORRENT_GUARD" == 'y' ]]; then
	...
	if [[ "$IPV6" == 'y' ]]; then
		ipset create antizapret-torrent6 hash:ip timeout 60 family inet6 -exist
		ip6tables -w -I FORWARD 2 -s $VPN_NET6 -p tcp -m string --string 'GET ' --algo kmp --to 100 -m string --string 'info_hash=' --algo bm -m string --string 'peer_id=' --algo bm -m string --string 'port=' --algo bm -j SET --add-set antizapret-torrent6 src --exist
		ip6tables -w -I FORWARD 3 -s $VPN_NET6 -p udp -m string --string 'BitTorrent protocol' --algo kmp --to 100 -j SET --add-set antizapret-torrent6 src --exist
		ip6tables -w -I FORWARD 4 -s $VPN_NET6 -p udp -m string --string 'd1:ad2:id20:' --algo kmp --to 100 -j SET --add-set antizapret-torrent6 src --exist
		ip6tables -w -I FORWARD 5 -s $VPN_NET6 -m set --match-set antizapret-torrent6 src -j DROP
	fi
fi
# Restrict forwarding
	...
	if [[ "$IPV6" == 'y' ]]; then
		{
			...
			if [[ -f result/forward-ips6.txt ]]; then
				while read -r line; do
					echo "add antizapret-forward6 $line"
				done < result/forward-ips6.txt
			fi
		} | ipset restore
		ip6tables -w -I FORWARD 2 -s $ANTIZAPRET_NET6 -m connmark --mark 0x1 -m set ! --match-set antizapret-forward6 dst -j DROP
	fi
fi
# Drop forwarding
{
...
if [[ "$IPV6" == 'y' ]]; then
	{
		echo 'create antizapret-drop6 hash:net family inet6 -exist'
		echo 'flush antizapret-drop6'
		if [[ -f result/drop-ips6.txt ]]; then
			while read -r cidr; do
				echo "add antizapret-drop6 $cidr"
			done < result/drop-ips6.txt
		fi
	} | ipset restore
	ip6tables -w -I FORWARD 2 -s $CLIENT_NET6 -m set --match-set antizapret-drop6 dst -j DROP
fi
...
if [[ "$CLIENT_ISOLATION" == 'y' ]]; then
	...
	if [[ "$IPV6" == 'y' ]]; then
		if [[ "$ANTIZAPRET_OUT_INTERFACE" == "$VPN_OUT_INTERFACE" ]]; then
			ip6tables -w -I FORWARD 2 ! -i $ANTIZAPRET_OUT_INTERFACE -d $CLIENT_NET6 -j DROP
		else
			ip6tables -w -I FORWARD 2 ! -i $ANTIZAPRET_OUT_INTERFACE -d $ANTIZAPRET_NET6 -j DROP
			ip6tables -w -I FORWARD 3 ! -i $VPN_OUT_INTERFACE -d $VPN_NET6 -j DROP
		fi
		ip6tables -w -I INPUT 2 -s $CLIENT_NET6 -p tcp ! --dport 53 -j DROP
		ip6tables -w -I INPUT 3 -s $CLIENT_NET6 -p udp ! --dport 53 -j DROP
	fi
fi
# SSH protection
...
if [[ "$ATTACK_PROTECTION" == 'y' ]]; then
	...
	iptables -w -I INPUT 6 -i $DEFAULT_INTERFACE -m conntrack --ctstate NEW -j SET --add-set antizapret-watch src,dst --exist
	{
		...
		if [[ -f result/allow-ips6.txt ]]; then
			while read -r line; do
...
{
	echo 'create antizapret-deny6 hash:net family inet6 -exist'
	echo 'flush antizapret-deny6'
	if [[ -f result/deny-ips6.txt ]]; then
		while read -r cidr; do
			echo "add antizapret-deny6 $cidr"
		done < result/deny-ips6.txt
	fi
} | ipset restore
ip6tables -w -I INPUT 2 -i $DEFAULT_INTERFACE -m set --match-set antizapret-deny6 src -j DROP
...
if [[ "$OPENVPN_BACKUP_TCP" == 'y' ]]; then
	...
	if [[ "$IPV6" == 'y' ]]; then
		ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p tcp --dport 80 -j REDIRECT --to-ports 50080
		ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p tcp --dport 443 -j REDIRECT --to-ports 50443
		ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p tcp --dport 504 -j REDIRECT --to-ports 50443
		ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p tcp --dport 508 -j REDIRECT --to-ports 50080
	fi
fi
...
if [[ "$OPENVPN_BACKUP_UDP" == 'y' ]]; then
	...
	if [[ "$IPV6" == 'y' ]]; then
		ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p udp --dport 80 -j REDIRECT --to-ports 50080
		ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p udp --dport 443 -j REDIRECT --to-ports 50443
		ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p udp --dport 504 -j REDIRECT --to-ports 50443
		ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p udp --dport 508 -j REDIRECT --to-ports 50080
	fi
fi
...
if [[ "$WIREGUARD_BACKUP" == 'y' ]]; then
	...
	if [[ "$IPV6" == 'y' ]]; then
		ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p udp --dport 540 -j REDIRECT --to-ports 51443
		ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p udp --dport 580 -j REDIRECT --to-ports 51080
	fi
fi
...
if [[ "$IPV6" == 'y' ]]; then
	ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p udp --dport 52080 -j REDIRECT --to-ports 51080
	ip6tables -w -t nat -A PREROUTING -i $DEFAULT_INTERFACE -p udp --dport 52443 -j REDIRECT --to-ports 51443
fi
...
if [[ "$RESTRICT_FORWARD" == 'y' ]]; then
	...
	if [[ "$IPV6" == 'y' ]]; then
		ip6tables -w -t nat -A PREROUTING -s $ANTIZAPRET_NET6 ! -d $FAKE_NET6 -j CONNMARK --set-mark 0x1
	fi
fi
...
if [[ "$IPV6" == 'y' ]]; then
	...
	ip6tables -w -t nat -A PREROUTING -s $ANTIZAPRET_NET6 -d $FAKE_NET6 -j ANTIZAPRET-MAPPING
fi
# SNAT/MASQUERADE VPN
...
if [[ "$IPV6" == 'y' ]]; then
	if [[ "$ANTIZAPRET_OUT_INTERFACE" == "$VPN_OUT_INTERFACE" && "$ANTIZAPRET_OUT_IP6" == "$VPN_OUT_IP6" ]]; then
		if [[ -z "$ANTIZAPRET_OUT_IP6" ]]; then
			ip6tables -w -t nat -A POSTROUTING -s $CLIENT_NET6 -o $ANTIZAPRET_OUT_INTERFACE -j MASQUERADE
		else
			ip6tables -w -t nat -A POSTROUTING -s $CLIENT_NET6 -o $ANTIZAPRET_OUT_INTERFACE -j SNAT --to-source $ANTIZAPRET_OUT_IP6
		fi
	else
		if [[ -z "$ANTIZAPRET_OUT_IP6" ]]; then
			ip6tables -w -t nat -A POSTROUTING -s $ANTIZAPRET_NET6 -o $ANTIZAPRET_OUT_INTERFACE -j MASQUERADE
		else
			ip6tables -w -t nat -A POSTROUTING -s $ANTIZAPRET_NET6 -o $ANTIZAPRET_OUT_INTERFACE -j SNAT --to-source $ANTIZAPRET_OUT_IP6
		fi
		if [[ -z "$VPN_OUT_IP6" ]]; then
			ip6tables -w -t nat -A POSTROUTING -s $VPN_NET6 -o $VPN_OUT_INTERFACE -j MASQUERADE
		else
			ip6tables -w -t nat -A POSTROUTING -s $VPN_NET6 -o $VPN_OUT_INTERFACE -j SNAT --to-source $VPN_OUT_IP6
		fi
	fi
fi
