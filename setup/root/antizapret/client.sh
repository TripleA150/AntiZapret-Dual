...
SERVER_IP6="$(ip -6 route get 2606:4700:4700::1111 2>/dev/null | grep -oP 'src \K\S+')"
if [[ -z "$SERVER_IP" && -z "$SERVER_IP6" ]]; then
	echo 'Default IPv4 and IPv6 addresses not found!'
	exit 3
fi
...
# Адреса шлюзов WireGuard/AmneziaWG в IPv6
ANTIZAPRET_WG_IP6="${ANTIZAPRET_WG_IP6:-fd29:0:0:8::1}"
VPN_WG_IP6="${VPN_WG_IP6:-fd28:0:0:8::1}"
...
setServerHost_FileName(){
	if [[ -z "$1" ]]; then
		SERVER_HOST="${SERVER_IP:-$SERVER_IP6}"
	else
		SERVER_HOST="$1"
	fi
...
# Приводим адреса в конфигурации WireGuard/AmneziaWG в соответствие с настройкой IPV6
# IPv6-адрес клиента получаем из последнего октета его IPv4-адреса
syncWireGuardIPv6(){
	local conf="$1"
	local gw6="$2"
	local prefix6="${gw6%::*}::"
	[[ -f "$conf" ]] || return 0
	awk -v ipv6="$IPV6" -v gw6="$gw6" -v prefix6="$prefix6" '
	/^Address = / || /^AllowedIPs = / {
		split($0, kv, " = ")
		ip = kv[2]
		sub(/,.*/, "", ip)
		...
		if (kv[1] == "Address") {
			print kv[1] " = " ip ", " gw6 "/64"
		} else {
			split(ip, octets, ".")
			host = octets[4]
			sub(/\/.*/, "", host)
			print kv[1] " = " ip ", " prefix6 host "/128"
		}
		next
	}
	{ print }
...
addWireGuard(){
	...
	# IPv6-адрес в Endpoint записывается в квадратных скобках
	if [[ "$SERVER_HOST" == *:* ]]; then
	...
	IPS6="$(cat /etc/wireguard/ips6 2>/dev/null || true)"

	syncWireGuardIPv6 /etc/wireguard/antizapret.conf "$ANTIZAPRET_WG_IP6"
	syncWireGuardIPv6 /etc/wireguard/vpn.conf "$VPN_WG_IP6"
	...
	if [[ -n "$CLIENT_BLOCK" ]]; then
		...
		CLIENT_IP6="$(echo "$CLIENT_BLOCK" | grep 'AllowedIPs =' | cut -d '=' -f 2- | tr ',' '\n' | grep ':' | sed 's/ //g' | cut -d '/' -f 1)"
		...
		CLIENT_IP6=
		CLIENT_ALLOWED_IPS="${CLIENT_IP}/32"
		if [[ "$IPV6" == 'y' ]]; then
			CLIENT_IP6="${ANTIZAPRET_WG_IP6%::*}::$i"
			CLIENT_ALLOWED_IPS="${CLIENT_ALLOWED_IPS}, ${CLIENT_IP6}/128"
		fi
...
backup(){
	...
	BACKUP_FILE="/root/antizapret/backup-${SERVER_IP:-${SERVER_IP6//:/-}}.tar.gz"
