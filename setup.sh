...
until [[ "$OPENVPN_TCP_ENABLE" =~ (y|n) ]]; do
	read -rp 'Enable OpenVPN TCP? [y/n]: ' -e -i y OPENVPN_TCP_ENABLE
done
echo
...
if [[ -n "$DEFAULT_IP6" ]]; then
	...
	echo 'IPv6 will be enabled inside all VPN, clients will be able to connect over IPv6'
	IPV6_DEFAULT=y
else
	echo "No global IPv6 address detected on $DEFAULT_INTERFACE!"
	echo 'IPv6 can still be enabled inside all VPN, but IPv6 sites will stay unreachable'
	IPV6_DEFAULT=n
fi
until [[ "$IPV6" =~ (y|n) ]]; do
	read -rp 'Enable IPv6 support? [y/n]: ' -e -i $IPV6_DEFAULT IPV6
done
echo
...
until [[ "$OPENVPN_DUPLICATE" =~ (y|n) ]]; do
	read -rp 'Allow multiple clients connecting to OpenVPN using same profile file (*.ovpn)? [y/n]: ' -e -i n OPENVPN_DUPLICATE
done
echo
...
until [[ "$ATTACK_PROTECTION" =~ (y|n) ]]; do
	read -rp 'Enable network attack protection? [y/n]: ' -e -i n ATTACK_PROTECTION
done
echo
...
until [[ "$TORRENT_GUARD" =~ (y|n) ]]; do
	read -rp $'Enable torrent guard for \001\e[1;32m\002full VPN\001\e[0m\002? [y/n]: ' -e -i n TORRENT_GUARD
done
echo
...
do
	...
	if [[ "$IPV6" == 'y' ]]; then
		[[ -n $(getent ahosts "$OPENVPN_HOST") ]] && break
	else
...
do
	...
	if [[ "$IPV6" == 'y' ]]; then
		[[ -n $(getent ahosts "$WIREGUARD_HOST") ]] && break
	else
...
until [[ "$CLOUDFLARE_INCLUDE" =~ (y|n) ]]; do
	read -rp $'Include Cloudflare IPs in \001\e[1;32m\002AntiZapret VPN\001\e[0m\002? [y/n]: ' -e -i n CLOUDFLARE_INCLUDE
done
echo
...
until [[ "$WHATSAPP_INCLUDE" =~ (y|n) ]]; do
	read -rp $'Include WhatsApp IPs in \001\e[1;32m\002AntiZapret VPN\001\e[0m\002? [y/n]: ' -e -i n WHATSAPP_INCLUDE
done
echo
...
FAKE_NET6=
ANTIZAPRET_WG_IP6=
VPN_WG_IP6=" > /tmp/antizapret/setup/root/antizapret/setup
...
# Включаем или выключаем поддержку IPv6
# Строки с префиксом #ipv6# нужны только при включённом IPv6, с префиксом #noipv6# - только при выключенном
IPV6_FILES=(
	/etc/openvpn/server/*.conf
	/etc/openvpn/server/scripts/client-connect.sh
	/etc/openvpn/client/templates/*.conf
	/etc/wireguard/templates/*.conf
	/etc/sysctl.d/99-antizapret.conf
)
if [[ "$IPV6" == 'y' ]]; then
	sed -i -e 's/^#ipv6#//' -e '/^#noipv6#/d' "${IPV6_FILES[@]}"
	sed -i 's/^local ipv6 = false/local ipv6 = true/' /etc/knot-resolver/kresd.conf
	# Форвардинг отключает приём Router Advertisement, возвращаем его на внешнем интерфейсе
	echo "net.ipv6.conf.$DEFAULT_INTERFACE.accept_ra=2" >> /etc/sysctl.d/99-antizapret.conf
else
	sed -i -e '/^#ipv6#/d' -e 's/^#noipv6#//' "${IPV6_FILES[@]}"
fi
sysctl -p /etc/sysctl.d/99-antizapret.conf || true
